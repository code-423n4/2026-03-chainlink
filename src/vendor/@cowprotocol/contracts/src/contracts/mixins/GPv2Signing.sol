...
abstract contract GPv2Signing {
  ...
  function allocateRecoveredOrder() internal pure returns (RecoveredOrder memory recoveredOrder) {
  ...
  function recoverOrderFromTrade(
    ...
    (bytes32 orderDigest, address owner) = recoverOrderSigner(order, signingScheme, trade.signature);

    recoveredOrder.uid.packOrderUidParams(orderDigest, owner, order.validTo);
  ...
  function recoverPreSigner(
    ...
    require(preSignature[orderUid] == PRE_SIGNED, "GPv2: order not presigned");
  }
}
