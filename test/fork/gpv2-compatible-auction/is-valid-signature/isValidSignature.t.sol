...
contract GPV2CompatibleAuction_IsValidSignatureForkTest is BasePriceManagerForkTest {
  uint256 private constant USDC_AMOUNT = 100_000e6;

  address private immutable i_solver = makeAddr("solver");
  GPv2Authentication private immutable i_gpv2Authentication = GPv2Authentication(GP_V2_ALLOWLIST_AUTHENTICATION);
  IGPV2Settlement private immutable i_gpv2Settlement = IGPV2Settlement(GP_V2_SETTLEMENT);

  IERC20[] private s_tokens;
  uint256[] private s_clearingPrices;
  GPv2Trade.Data[] private s_trades;
  GPv2Interaction.Data[][3] private s_interactions;
  GPv2Order.Data private s_order;
  uint256 private s_minBuyAmount;
  ...
  function setUp() external {
    ...
    s_trades.push(
      GPv2Trade.Data({
        ...
        signature: abi.encodePacked(address(s_auction), abi.encode(s_order))
      })
    );
